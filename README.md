# davidlb025.github.io

Mi página personal / portafolio profesional, alojada con GitHub Pages.

---

## 🚀 Acerca de

Mi **portafolio personal**, para exponer mis proyectos, habilidades y experiencia. Pensado como carta de presentación profesional.

---

## 🛠 Tecnologías utilizadas

- **Frontend**: HTML5, CSS3, JavaScript  
- **Backend**: Python, Node.js, PHP, Bases de datos SQL y NoSQL
- **Desarrollo de Software**:  Python, C++, POO, integración de APIs.  
- **IA / Machine Learning**: PyTorch, modelos de LLM, Deep Learning  
- **Frameworks / Librerías**: Flask, FastAPI, Django, Tkinter, QtCreator...
- **Bases de datos**: SQL, NoSQL  
- **Herramientas**: Git, GitHub, Docker, Qt Creator, Figma

---

## 📂 Estructura del proyecto

La estructura del repositorio es la típica para una página estática alojada con GitHub Pages:

- `index.html` — Página principal  
- `assets/css/` — Estilos  
- `assets/js/` — Scripts  
- `assets/img/` — Imágenes (actualmente *placeholders* en link)  
- `projects/` — Carpeta con los ejemplos de proyectos  
- `lang/` — Traducciones  

---

## ⚠️ Estado actual

- Versión **preliminar** del sitio  
- Las imágenes son **temporales (placeholders)**  
- Falta implementar la **lógica de traducción** de los proyectos  
- No hay enlaces externos definitivos todavía  

